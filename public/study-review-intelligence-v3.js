(() => {
  ...
  const STORE_KEY = 'liquid-glass-ai-study-review-v3';
  ...
  const newState = () => ({ version: 3, loginDays: {}, noteOpens: {}, sessions: [], pending: [], completedReviews: [], latestDecision: null });
  let state = safeParse(localStorage.getItem(STORE_KEY), null);
  if (!state || typeof state !== 'object') state = newState();
  state.loginDays ||= {}; state.noteOpens ||= {}; state.sessions ||= {}; state.pending ||= []; state.completedReviews ||= [];
  ...
  const recordLogin = () => {
    const key = todayKey();
    if (!state.loginDays[key]) state.loginDays[key] = { firstSeenAt: Date.now(), visits: 1 };
    save();
  };
  ...
  const openReview = async () => {
    ...
    const box = modal(); showLoading(box); const payload = buildPayload(pending);
