(() => {
  ...
  const STORE_KEY = 'liquid-glass-ai-study-review-v1';
  ...
  const readLearning = () => safeParse(localStorage.getItem(LEARNING_KEY), null) || {
    notes: {}, reviews: {}, activity: { studyMinutes: 0, sessions: 0, viewedCourses: 0, viewedCollections: 0, lastLogin: null },
  };

  const loginToday = () => {
    const today = todayKey();
    if (!state.loginDays[today]) state.loginDays[today] = { firstSeenAt: Date.now(), visits: 0 };
    state.loginDays[today].visits += 1;
    save();
  };
  ...
  const recordNoteOpen = (title, course = '') => {
    ...
    const today = todayKey();
    state.noteOpens[today] ||= {};
    const key = course ? `${course} · ${cleanTitle}` : cleanTitle;
    state.noteOpens[today][key] = (state.noteOpens[today][key] || 0) + 1;
    save();
  };
  ...
  const readSessionMinutes = (screen) => {
    const value = screen.querySelector('.session-stat-grid .session-stat:first-child strong')?.textContent || '';
    const match = value.match(/(\d+(?:\.\d+)?)\s*m/i);
    if (match) return Number(match[1]);
    return 0;
  };

  const readRestMinutes = (screen) => {
    const text = screen.querySelector('.session-stat-grid .session-stat:nth-child(3) strong')?.textContent || '';
    const match = text.match(/(\d+(?:\.\d+)?)\s*m/i);
    return match ? Number(match[1]) : 5;
  };

  const readPlanTarget = (screen) => {
    const text = screen.querySelector('.study-plan-target')?.textContent || '';
    const match = text.match(/(\d+(?:\.\d+)?)\s*hr/i);
    return match ? Number(match[1]) : null;
  };

  const sessionFingerprint = (screen) => {
    const { course, concept } = readCourseAndGoal(screen);
    return `${course}|${concept}|${screen.dataset.studyReviewStartedAt || ''}`;
  };

  const announceCountdown = (releaseAt, restMinutes) => {
    const launch = document.querySelector('.study-review-launcher');
    if (!launch) return;
    launch.hidden = false;
    launch.classList.remove('is-ready');
    launch.classList.add('is-resting');
    launch.dataset.releaseAt = String(releaseAt);
    const detail = launch.querySelector('.study-review-detail');
    if (detail) detail.textContent = `${restMinutes} min recovery · review unlocks after rest`;
    const title = launch.querySelector('.study-review-title');
    if (title) title.textContent = 'Recovery in progress';
  };

  const markSessionComplete = (screen) => {
    const status = screen.querySelector('.session-live-pill')?.textContent?.trim() || '';
    const completed = /session complete/i.test(status) || /complete/i.test(screen.querySelector('.session-clock small')?.textContent || '');
    if (!completed) return;
    const fingerprint = sessionFingerprint(screen);
    if (screen.dataset.studyReviewCompleted === fingerprint) return;
    const minutes = readSessionMinutes(screen);
    if (minutes < MIN_REVIEW_FOCUS_MINUTES) return;

    screen.dataset.studyReviewCompleted = fingerprint;
    const { course, concept } = readCourseAndGoal(screen);
    const restMinutes = readRestMinutes(screen);
    const targetHours = readPlanTarget(screen);
    const session = {
      id: `session-${Date.now()}`,
      startedAt: Number(screen.dataset.studyReviewStartedAt || Date.now()),
      ...
      concept,
      targetHours,
      mode: screen.querySelector('.session-mode-tabs .is-selected')?.textContent?.trim() || 'Focus',
      notesOpenedBefore: getTodayNoteOpens(),
    };
    ...
    state.pending.push({
      ...
      releaseAt: Date.now() + restMinutes * 60 * 1000,
      restMinutes,
      ready: false,
      session,
    });
    ...
    save();
    announceCountdown(Date.now() + restMinutes * 60 * 1000, restMinutes);
    scheduleReadyCheck();
  };

  const captureStudyStart = (screen) => {
    if (!screen.dataset.studyReviewStartedAt) screen.dataset.studyReviewStartedAt = String(Date.now());
  };

  const scheduleReadyCheck = () => {
    const next = state.pending.filter((item) => !item.ready).sort((a, b) => a.releaseAt - b.releaseAt)[0];
    if (!next) return;
    const delay = Math.min(60000, Math.max(500, next.releaseAt - Date.now()));
    window.clearTimeout(scheduleReadyCheck.timer);
    scheduleReadyCheck.timer = window.setTimeout(() => { markReady(); scheduleReadyCheck(); }, delay);
  };
  ...
  const markReady = () => {
    ...
    state.pending.forEach((item) => {
      if (!item.ready && item.releaseAt <= now) { item.ready = true; changed = true; }
    });
    if (changed) save();
    renderLauncher();
  };
  ...
  const latestReady = () => {
    ...
    return state.pending
      ...
      .sort((a, b) => b.releaseAt - a.releaseAt)[0] || null;
  };

  const buildPayload = (pending) => {
    const learning = readLearning();
    const today = todayKey();
    const daySessions = state.sessions.filter((item) => todayKey(new Date(item.completedAt)) === today);
    const concepts = [...new Set(daySessions.map((item) => item.concept).filter(Boolean))];
    const openMap = state.noteOpens[today] || {};
    const noteTitles = Object.keys(openMap);
    const reviewed = Object.values(learning.reviews || {}).filter((item) => item && item.reviews > 0);
    const recallAccuracy = reviewed.length
      ? Math.round((reviewed.reduce((sum, item) => sum + ((item.correct || 0) / Math.max(1, item.reviews || 1)), 0) / reviewed.length) * 100)
      : 0;
    return {
      today,
      login: state.loginDays[today] || null,
      daily: {
        studyMinutes: daySessions.reduce((sum, item) => sum + (item.focusMinutes || 0), 0),
        completedSessions: daySessions.length,
        concepts,
        notesOpened: getTodayNoteOpens(),
        noteTitles,
  ...
  const deterministicAnalysis = (payload) => {
    const minutes = payload.daily.studyMinutes;
    const current = payload.currentSession || {};
    const notes = payload.daily.notesOpened;
    const recall = payload.recall.accuracy;
    const focus = current.concept || current.course || 'today’s study';
    ...
    return {
      summary: insight,
      strengths: minutes >= 45 ? 'You completed a meaningful focus block.' : 'You still created useful study momentum.',
      weakArea: recall && recall < 75 ? focus : (notes >= 8 ? 'Independent recall' : 'Not enough signal yet'),
      nextReview: recall && recall < 75 ? 'Tomorrow' : 'In 2 days',
      questions: [
  ...
  const callAI = async (payload, phase, answers = null) => {
    ...
    if (!data || typeof data !== 'object') throw new Error('Invalid AI review response');
    return data;
  };
  ...
  const ensureLauncher = () => {
    ...
    launcher.className = 'study-review-launcher';
  ...
  const renderLauncher = () => {
    ...
    if (pending) {
      launcher.hidden = false;
      launcher.classList.add('is-resting');
  ...
  const createModal = () => {
    ...
    backdrop.className = 'study-review-backdrop';
  ...
  const setModalLoading = (modal) => {
    modal.querySelector('[data-review-body]').innerHTML = `<div class="study-review-loading"><span class="study-review-spinner"></span><strong>Analyzing today’s activity…</strong><small>Study time · concepts · notes · previous recall</small></div>`;
  };
  ...
  const submitReview = async (event, modal, pending, payload, data, questions) => {
    ...
    submit.textContent = 'Saving and deciding…';
    ...
    try {
      const result = await callAI(payload, 'decision', answers);
  ...
  const openReview = async () => {
    ...
    try {
      const data = await callAI(payload, 'questions');
      renderQuestions(modal, pending, payload, data.questions?.length === 3 ? data : fallback);
    } catch {
      renderQuestions(modal, pending, payload, fallback);
    }
  };

  const scan = () => {
    loginToday();
    document.querySelectorAll('.study-session-screen').forEach((screen) => { captureStudyStart(screen); markSessionComplete(screen); });
    markReady();
    renderLauncher();
    scheduleReadyCheck();
  };
  ...
  const observer = new MutationObserver(() => {
    ...
    scan.timer = window.setTimeout(scan, 160);
  });
  ...
  window.addEventListener('storage', (event) => {
    if (event.key === STORE_KEY) {
      state = safeParse(event.newValue, state) || state;
      renderLauncher();
    }
  });
